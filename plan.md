...
- 第 11 课 掉落分层 + 拾取分化（对齐搜打撤设想与 GDD 191/201/718 行）：经验球/金币（复用 PickUpMagnet 磁吸）+ 敌人分级掉落表（普通=经验球为主/精英=蓝装地板/Boss 专属表）+ 保底修正（先掷品类，装备掷骰才计保底）+ IInteractable 交互系统（E 键：地面装备摘磁吸改为近身提示+按键拾取，收货口 OnCollected 不变）
- 第 12 课 容器搜刮：宝箱/隐藏宝箱（必出装备，隐藏箱绿装地板，GDD 11.5/11.6；宝箱实现 IInteractable，复用 E 交互）
- 第 13 课 合并升级 + 第一条邻接联动
