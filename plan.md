...
- 第 36 课（8/08，原 8/12~8/13 计划提前完成）Build 与演示包 ✅：Build 前全包扫描通过；修正两个场景 UIInputModule actions 引用；关闭 Windows Build Profile 的 Development/Autoconnect Profiler；Player Settings 调整为 1600×900 Windowed、Resizable、Version 0.2.0；正式 Windows 包输出到 Builds/BackpackSurvivor_v0.2_Windows 并完成独立 exe 验收；Build 输出被 .gitignore 忽略；文档 Docs/第36课-Build与演示包.md。
- 第 37 课（8/13~8/15）作品材料 ◀ 当前：README、录屏脚本、截图清单、简历项目描述、技术亮点提炼；把“我做了什么、难点在哪里、怎么验证”讲清楚。
- 第22课后内容系统性价比队列：P0 已完成 = 数据源保真、掉落入口可跑、三类自动武器基础差异、FireRateBoost 芯片真实收益、精英/宝箱节奏、史诗宝箱阶段展示、金币掉落/HUD、单件价值显示、背包价值结算、合并升级收益、物品详情 Tooltip、弹夹超模回调、怪物血量随波次成长、伤害显示统一、宝箱距离提示、首轮 15 分钟可玩验证、旋转邻接方向修正、武器稀有度/等级差异、攻击芯片效果、物品图标与背包可读性、主菜单场景、场景氛围包装、完整通关验收、玩法说明入口、主动瞄准一致性修正、Profiler 快扫与 Build 颜色异常修复、正式 Windows Build 独立验收；P1 继续 = 作品材料；P2 Demo 后 = 新手目标提示、金币结算页取舍、危险区表现强化、完整物品配置 SO、Gold 商店、出售/撤离经济、全套词缀和复杂互斥规则
...
### 阶段 4：V0.3 内容深度与长期留存（Demo 后扩展）

详见：`Docs/V0.3开发计划.md`。

V0.3 不推倒 V0.2 主干，重点围绕“更多选择、更强反馈、更明确目标和局外留存”推进。

- 第 38 课 V0.3 版本闸门与内容基线：确认升级池、邻接池、物品池、音频、Boss、存档的边界与优先级。
- 第 39 课 三选一升级扩容一期：升级池从 3 个扩到 8~12 个，覆盖攻击、生存、机动、搜刮、经济方向。
- 第 40 课 升级生成器权重与体验优化：权重、等级门槛、互斥、不重复和 UI 分类表达。
- 第 41 课 邻接效果架构升级：梳理 Stackable / Exclusive / PerWeapon / Global / Capped 策略，避免 Resolver 和 Activator 膨胀。
- 第 42 课 新邻接效果实装：优先做 CritBoost、RangeBoost、ShieldLink 等能被玩家感知的真实收益。
- 第 43 课 内容池扩展：新增芯片、武器或武器变体、高价值收集品，并接入掉落表、图标和 Tooltip。
- 第 44 课 基础音频系统与 BGM：从零散 SFX 升级到 SFX/Music 分层，补主菜单/战斗/Boss 音乐和关键反馈音。
- 第 45 课 敌人寻路与群体移动优化：从 EnemyAI 拆出移动模块，加分离/避让/低频重算，并用 Profiler 验证。
- 第 46 课 Boss 一期：Boss 出场、血条、基础 FSM、死亡奖励和结算链路。
- 第 47 课 Boss 二期：冲锋、弹幕、召唤小怪、阶段变化、Boss 音频。
- 第 48 课 本地存档与最高纪录：最高分、最高金币、最高背包价值、金色物品数量、主菜单纪录展示。
- 第 49 课 V0.3 平衡、验收与发布：完整试玩、Profiler 快扫、Build v0.3、README/Release/V0.3 复盘。
