...
- 8/11~8/12：做 15 分钟 Demo 的可读性和稳定性：新手目标提示、场景氛围、完整通关验收、Profiler 快扫。
...
- 第 29 课（8/08）武器稀有度与等级差异 ◀ 当前：解决“不同稀有度/等级武器只是同一模板”的问题；不急着做多套武器 prefab，先让 Item 的 Rarity/Level 影响激活出来的 AutoWeapon 数值（伤害、射速、射程或弹速择高性价比项），并在 Tooltip/HUD 中让玩家看懂“为什么这把枪更强”。
...
- 第 31 课（8/10）物品图标与背包可读性：低成本给主要物品补类别图标，不做完整美术系统；优先覆盖手枪/步枪/霰弹枪/弹夹芯片/攻击芯片/收集品/高价值物，稀有度仍用背景色，等级仍用角标，Tooltip 继续承载详情。
- 第 32 课（8/10）新手目标提示与局内可读性：开局提示、当前目标、背包满/可合并/可邻接/高价值物品提示、升级选择文案；少做教程系统，多做不会挡操作的轻提示。
- 第 33 课（8/10）主菜单与场景流：新增 MainMenu 场景，提供开始游戏、退出游戏；整理 Build Settings 场景顺序；Result 页退出可回主菜单，重开仍可直接重载 Run 场景；目标是别人打开 Build 后有完整入口，而不是直接掉进测试场。
- 第 34 课（8/11）场景氛围与演示包装：地面/边界/危险区可读性、基础光照与材质统一、掉落和宝箱视觉层级；目标是截图不寒酸，不追求大规模美术制作。
- 第 35 课（8/11~8/12）完整 15 分钟通关验收：按玩家视角完整跑 3 局，记录阻断 bug、节奏断点、数值过强/过弱；只修会影响演示的 bug，不开新系统。
- 第 36 课（8/12）Profiler 快扫与低风险优化：看 CPU/GPU/GC/对象池热点；只处理掉帧、GC 尖刺、对象未归零这类高风险问题，不做大重构。
- 第 37 课（8/13）Build 与演示包：Windows Build、分辨率/窗口模式、输入检查、干净启动场景；准备可交给别人直接运行的版本。
- 第 38 课（8/14~8/15）作品材料：README、录屏脚本、截图清单、简历项目描述、技术亮点提炼；把“我做了什么、难点在哪里、怎么验证”讲清楚。
- 第22课后内容系统性价比队列：P0 已完成 = 数据源保真、掉落入口可跑、三类自动武器基础差异、FireRateBoost 芯片真实收益、精英/宝箱节奏、史诗宝箱阶段展示、金币掉落/HUD、单件价值显示、背包价值结算、合并升级收益、物品详情 Tooltip、弹夹超模回调、怪物血量随波次成长、伤害显示统一、宝箱距离提示、首轮 15 分钟可玩验证、旋转邻接方向修正；P1 继续 = 武器稀有度/等级差异、攻击芯片效果、物品图标与背包可读性、新手目标提示、主菜单场景、场景氛围、完整通关验收；P2 Demo 后 = 金币结算页取舍、完整物品配置 SO、Gold 商店、出售/撤离经济、全套词缀和复杂互斥规则
- 可选增强挂账：附近战利品面板（塔科夫式 loot panel，背包打开时拖拽地面物品入包，纯 UI 层拖拽复用现有三态）
- 邻接系统挂账优先级（第 28 课后重排）：P1 DamageBoost 攻击芯片真实兑现 → P2 结算页/局内提示展示已生效芯片收益 → P2 CritBoost/BurningBullets 等第二类芯片 → P2 Resolver 层数上限策略数据化 → P2 物品/规则配置数据化 → P2 地面掉落保留运行时旋转态
