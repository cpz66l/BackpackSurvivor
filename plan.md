...
- 8/03~8/06：补齐 Demo 经济与构筑反馈：金币掉落/HUD、物品价值显示、背包总价值、合并升级收益、数值调参台。
- 8/07~8/09：做 15 分钟 Demo 的可读性和稳定性：新手目标提示、场景氛围、完整通关验收。
- 8/10~8/12：作品交付与低风险打磨：Profiler 快扫、Build、README、录屏脚本、简历项目描述。
...
- 第 24 课（8/03）金币掉落与局内经济 HUD ◀ 当前：补齐 DropCategory.Gold 分支；金币掉落物复用磁吸收集节奏；GameSession 或 RunEconomy 统计本局金币；RunHudView 显示金币；结算页显示金币收入。只做“金币可捡、可看、可结算”，不做商店。
- 第 25 课（8/04）背包价值与物品价值显示：ItemView 显示单件价值；背包面板显示当前背包总价值；RunResult/ResultView 接入背包携带价值；结算分数从击杀/等级扩展到“战斗表现 + 搜刮价值”。
- 第 26 课（8/05）合并升级收益兑现：同名同级 2→1 不只改等级显示，还要让 ScoreValue / EffectValue 随等级成长；ItemView 显示升级后的价值/效果；FireRateBoost 等现有消费侧读到升级后的效果值；保留纯数据层可测。
- 第 27 课（8/06）数值调参台与首轮平衡：做一个仅开发期使用的调参入口或集中配置区；调敌人血量、速度、刷怪频率、掉落率、经验曲线、金币量、物品价值、合并收益；目标是新手 8~12 分钟、熟悉后能通关。
- 第 28 课（8/07）新手目标提示与局内可读性：开局提示、当前目标、背包满/可合并/可邻接/高价值物品提示、升级选择文案；少做教程系统，多做不会挡操作的轻提示。
- 第 29 课（8/08）场景氛围与演示包装：地面/边界/危险区可读性、基础光照与材质统一、掉落和宝箱视觉层级；目标是截图不寒酸，不追求大规模美术制作。
- 第 30 课（8/09）完整 15 分钟通关验收：按玩家视角完整跑 3 局，记录阻断 bug、节奏断点、数值过强/过弱；只修会影响演示的 bug，不开新系统。
- 第 31 课（8/10）Profiler 快扫与低风险优化：看 CPU/GPU/GC/对象池热点；只处理掉帧、GC 尖刺、对象未归零这类高风险问题，不做大重构。
- 第 32 课（8/11）Build 与演示包：Windows Build、分辨率/窗口模式、输入检查、干净启动场景；准备可交给别人直接运行的版本。
- 第 33 课（8/12）作品材料：README、录屏脚本、截图清单、简历项目描述、技术亮点提炼；把“我做了什么、难点在哪里、怎么验证”讲清楚。
- 第22课后内容系统性价比队列：P0 已完成 = 数据源保真、掉落入口可跑、三类自动武器基础差异、FireRateBoost 芯片真实收益、精英/宝箱节奏、史诗宝箱阶段展示；P1 继续 = 金币掉落/HUD、背包价值结算、单件价值显示、合并升级收益、数值调参台、新手目标提示、场景氛围；P2 Demo 后 = 完整物品配置 SO、Gold 商店、出售/撤离经济、全套词缀和复杂互斥规则
- 可选增强挂账：附近战利品面板（塔科夫式 loot panel，背包打开时拖拽地面物品入包，纯 UI 层拖拽复用现有三态）
- 邻接系统挂账优先级（第 22 课后）：P1 结算页/局内提示展示已生效芯片收益 → P2 CritBoost/BurningBullets 等第二类芯片 → P2 Resolver 层数上限策略数据化 → P2 物品/规则配置数据化 → P3 旋转态接口方向策略
