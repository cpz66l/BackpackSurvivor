...
- 第 22 课（8/04）内容面铺开 ◀ 当前：至少 3 类可演示装备体验（手枪/步枪/霰弹枪或狙击枪），2~3 种掉落稀有度可感知；用最少美术替换和数值差异，让玩家觉得“东西变多了”；若接入 FireRateBoost/CritBoost/BurningBullets，则同步补 Resolver 的可堆叠效果分支
...
- 邻接系统挂账优先级（第 21 课后）：P1 基础芯片/可堆叠效果真实收益（FireRateBoost/CritBoost/BurningBullets 任选 1~2 个）→ P2 Resolver 可堆叠分支与堆叠上限 → P2 物品/规则配置数据化 → P3 旋转态接口方向策略
