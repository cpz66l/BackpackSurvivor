...
- v0.3.0.1 版本管理切换与内容基线：从课时制切换到版本号 / 模块号管理；明确 AI 协作边界、提交规则、文档规则和能力展示目标。
- v0.3.1 三选一升级候选池：升级池从 3 个扩到 8~12 个，覆盖攻击、生存、机动、搜刮、构筑方向，并加入权重、门槛和同轮不重复。
- v0.3.2 邻接效果架构升级：梳理 Stackable / Exclusive / PerWeapon / Global / Capped 策略，避免 Resolver 和 Activator 膨胀。
- v0.3.3 新邻接效果实装：优先做 CritBoost、RangeBoost、ShieldLink 等能被玩家感知的真实收益。
- v0.3.4 内容池扩展：新增芯片、武器或武器变体、高价值收集品，并接入掉落表、图标和 Tooltip。
- v0.3.5 基础音频系统与 BGM：从零散 SFX 升级到 SFX/Music 分层，补主菜单/战斗/Boss 音乐和关键反馈音。
- v0.3.6 敌人寻路与群体移动优化：从 EnemyAI 拆出移动模块，加分离/避让/低频重算，并用 Profiler 验证。
- v0.3.7 Boss 一期：Boss 出场、血条、基础 FSM、死亡奖励和结算链路。
- v0.3.8 Boss 二期：冲锋、弹幕、召唤小怪、阶段变化、Boss 音频。
- v0.3.9 本地存档与最高纪录：最高分、最高金币、最高背包价值、金色物品数量、主菜单纪录展示。
- v0.3.10 V0.3 平衡、验收与发布：完整试玩、Profiler 快扫、Build v0.3、README/Release/V0.3 复盘。
