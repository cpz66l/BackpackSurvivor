...
- 第 27 课（8/06）数值调参台与首轮平衡 ◀ 当前：做 Demo 期可快速调参的入口或集中配置区；重点回调 Magazine / FireRateBoost 的基础值、等级倍率和攻速上限；给怪物血量加入随波次或时间成长；同步调敌人速度、刷怪频率、精英率、经验曲线、金币量、物品价值、合并收益；目标是新手 8~12 分钟、熟悉后能通关。
...
- 第22课后内容系统性价比队列：P0 已完成 = 数据源保真、掉落入口可跑、三类自动武器基础差异、FireRateBoost 芯片真实收益、精英/宝箱节奏、史诗宝箱阶段展示、金币掉落/HUD、单件价值显示、背包价值结算、合并升级收益、物品详情 Tooltip；P1 继续 = 弹夹超模回调、怪物血量随波次成长、金币结算页取舍、数值调参台、新手目标提示、场景氛围；P2 Demo 后 = 完整物品配置 SO、Gold 商店、出售/撤离经济、全套词缀和复杂互斥规则
