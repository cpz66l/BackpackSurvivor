...
> Unity 3D 俯视角生存射击 Demo。玩家在 15 分钟单局中移动、射击、搜刮、整理背包，并通过武器激活、合并升级、邻接芯片、局内成长和本地纪录形成构筑循环；局外由 LLM 驱动跨局合同链，由可对话 NPC 下达任务并在结算后复盘战局。
...
![Workspace](https://img.shields.io/badge/Workspace-v0.4%20LLM%20Event%20Loop-blue)
![LLM](https://img.shields.io/badge/LLM-DeepSeek-4B6BFB)
...
当前工作区还接入了 **v0.4 LLM 事件驱动系统**：新增调度营地场景与跨局合同循环，由分层事件池按难度抽取合同、以纯 C# 判定器在结算时校验达成并推进难度层级；配套 NPC「小芯」覆盖营地对话、局内波次无线电与结算复盘三个对话面。系统的核心取舍是**判定权完全留在本地**，大模型只负责表达，断网状态下玩法链路完整可通关。设计与实施见 [技术方案](Docs/LLM事件驱动系统技术方案.md)、[施工流程](Docs/LLM事件驱动系统施工流程.md) 与 [施工复盘记录](Docs/LLM事件驱动系统施工复盘记录.md)。
...
| 当前工作区 | v0.4 美术重构 + LLM 事件驱动系统；Editor 验证完成，未打包发布 |
| 版本节点 | 2026-07-19开始；2026-08-20完成v0.3 Build验收；2026-09-08至09-09完成美术接入与Editor验证；2026-09-12至09-16完成LLM事件驱动系统 |
| 主要入口 | `Assets/BackpackSurvivor/Scenes/MainMenu/MainMenu.unity` |
| 调度营地 | `Assets/BackpackSurvivor/Scenes/Camp/Camp.unity`；每次出击必经 |
| 当前单局场景 | `Assets/BackpackSurvivor/Scenes/Run/01-Run_ArtFull.unity`；原`01-Run`保留 |
| 单局目标 | 15 分钟生存、成长、搜刮与结算，并达成当前合同 |
| v0.4发布前待验收 | Windows独立包完整单局、密集掉落性能、更多显示比例与 NPC 体验项人工验收 |
...
```text
主菜单
  -> 阅读玩法说明 / 调整设置 / 查看本地纪录 / 配置 AI NPC / 进入调度营地
  -> 调度营地：与 NPC 小芯对话，核对本局合同与目标
  -> 进入封锁区，移动、主动射击、自动武器战斗
  -> 击杀敌人，拾取经验、金币和装备
  -> 寻找并开启不同稀有度宝箱
  ...
  -> 波次切换时收到 NPC 的无线电短评
  -> 胜利或失败后进入结算：本地判定合同是否达成、写入战绩并生成小芯复盘
  -> 达成则解锁更高难度合同，完成任意终局合同即通关
```
...
- `QuestEventDefinition / QuestDatabase / QuestDrawer` 构成分层事件池，按难度抽取合同并排除已完成项；完成任意一层即可解锁下一层，完成任意终局合同即通关。
- `QuestEvaluator` 是纯 C# 判定器，在结算时扫描背包与局内战果判断合同是否达成；`SaveService.TryCompleteQuest` 在临时文件落盘成功后才替换内存，重复提交幂等，写档失败保留待完成状态。
- **判定权完全在本地**，模型不参与达成判定，断网状态下玩法链路完整可通关。
- 合同会冻结自身的定义版本与完整条件快照，事件资产后续发布新版本不影响已接受的合同。
- `LootContext` 区分合同局与常规局：任务局专属物品只在合同局进入候选池，过滤发生在抽签之前，并同时覆盖常规抽取与保底抽取两条路径。
- NPC「小芯」覆盖三个对话面：营地自由对话、局内波次无线电脉冲、结算情绪化复盘。三面共享同一份人设前缀，但历史各自独立；营地对话逐字流式显示。
- 本地意图路由把输入分为闲聊、事实、受限三类。受限输入（索要掉落概率、要求直接给物品、要求忽略规则等）**不产生任何网络调用**，直接由本地角色内措辞回复。
- 事实类问题必须经过白名单只读工具轮取回本地数据；具体数值一律以 `[[objective:0]]`、`[[progress:0]]`、`[[record:0]]` 等引用标记回填，模型不得自行书写数字或机制措辞。未替换干净的引用、阿拉伯数字与承诺性表述都会被校验拒绝，经有界重试后静默降级为本地文案。
- 工具按对话面收窄（营地 6 项、结算 4 项、脉冲 2 项），全部只读；玩家的对话输入不触发任何状态、背包或存档变更。
- 请求侧显式关闭 `thinking` 模式；营地走流式，脉冲与结算为一次性请求，脉冲带阶段序号与 TTL，过期结果直接丢弃。
- 主菜单提供「AI NPC 设置」，可配置总开关、模型 ID、密钥与四项额度；密钥通过环境变量提供，不写入仓库与包体，日志中全程掩码。
- `QuestTelemetry` 以 JSONL 记录合同结果、开箱品质分桶、精英击杀与任务物品产出，用于难度校准；当前数值依据一次完整对局（900 秒、等级 51、总击杀 1,996、精英 556、背包价值 105,750）校准。
...
- ScriptableObject 配置
- JSON 本地存档
- DeepSeek API（`deepseek-flash`，OpenAI 兼容格式，关闭 thinking 模式）
- Unity `Awaitable` 异步与流式响应处理
- Git / GitHub
...
│  │     ├─ Quest/                           # 事件池、合同判定、抽签、埋点与营地控制
│  │     ├─ Npc/                             # NPC 人设、意图路由、对话服务与流式表现
│  │     └─ Presentation/                    # HUD、背包、菜单、升级、音频、天气与掉落展示
│  ├─ Packages/
│  └─ ProjectSettings/
├─ Tools/                                    # 资产制作与编辑器辅助工具
├─ Docs/                                     # 版本复盘、LLM 设计与施工记录、世界观与角色设定、美术来源与资源证据
...
| LLM 判定与校验 | 判定器、意图路由与文本校验器均为纯 C#，可脱离引擎验证；EditMode 58/58 通过 |
| LLM 真实调用 | 完成真实 DeepSeek 调用审计：询问最高带回价值时调用 `get_player_records` 一次并回填本地存档数值，无失败与回退；受限输入确认零网络调用 |
| v0.4发布前待补 | Windows完整15分钟单局、密集掉落GPU性能、更多显示比例、NPC 三面体验人工采样；未承诺FPS提升 |
...
- [LLM 事件驱动系统技术方案](./Docs/LLM事件驱动系统技术方案.md)
- [LLM 事件驱动系统施工流程](./Docs/LLM事件驱动系统施工流程.md)
- [LLM 事件驱动系统施工复盘记录](./Docs/LLM事件驱动系统施工复盘记录.md)
- [世界观设定集](./Docs/世界观设定集.md)
- [NPC 人物设定](./Docs/NPC人物设定.md)
...
- 将事件池从手写资产改为条件组合式生成，扩大合同内容量并保持可完成性校验。
- 为 NPC 增加带摘要压缩的长期记忆，在控制上下文与成本的前提下提升陪伴感。
- 补齐营地、局内脉冲与结算三面的真实体验人工采样，并按反馈调整语气与事实引用比例。
...
- 能在游戏系统中引入大模型能力并守住工程边界：把判定权留在本地、用只读工具提供事实、以引用协议约束输出，保证断网可玩与结果可复现。
- 能围绕玩家体验迭代：战斗反馈、背包可读性、伤害一致性、数值平衡、目标提示、设置和 Build 稳定性。
- 能使用 Git、Profiler、Bug 记录和版本复盘沉淀工程过程，并把项目经验转化为可面试表达的项目重点。

## 项目说明

本仓库用于个人学习、玩法验证与求职作品展示。v0.3提供已发布的Windows Demo；当前工作区记录v0.4美术重构与LLM事件驱动系统的实现和Editor验证成果，但不代表v0.4完整版本已完成。项目同时展示设计取舍、AI辅助制作过程、系统边界、资源控制和验证范围。
