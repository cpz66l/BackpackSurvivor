...
## S8 用户反馈修复 · 默认真实对话与主菜单配置消费（2026-09-13）

**核心链路与目的**：主菜单已保存开关/模型 → 三面 NpcDialogueService → 营地实时回复与模式状态。修复普通 Play 默认走固定 Mock、闲聊强制查询合同、回复无可见上下文的问题。

**技术选择**：NpcPersona 默认关闭 Mock；服务每次请求读取总开关和模型，关闭时三面均在传输前返回，营地保留本地简报且禁用输入，波次/结算保留现有本地显示。闲聊直接流式回复，事实问询保留工具轮、白名单和字段引用；逐句支持问号/感叹号。营地保留最近四轮可见记录，只在本会话内存存在。状态区显示在线/关闭/回退，开发审计显示实际传输、所选模型、原始响应和工具执行。

**现场发现与修正**：默认入口的多轮真实回归暴露了严格数值筛选对“咖啡浓一点”“稳一点总没坏处”的误判。只对列出的形容词加“一点”形式做程度表达豁免，其他守卫仍校验原文；新增正反例测试。模型提示同步实际的计数/机制措辞限制，避免中文修辞计数反复触发降级。早期失败记录保留在 Feedback/npc-live-attempt*.txt，不能把一次最终通过解释为长期零失败。

**改动文件**：NpcDialogueService.cs、NpcPersona.cs、NpcPersona.asset、CampController.cs、NpcCore.cs、NpcBindingTests.cs、QuestS8SafetyAudit.cs、NpcLiveFeedbackAudit.cs 与 meta、两份方案、Feedback 证据。

**验证结果（本地）**：17 组服务场景通过，含三面关闭零请求、无额度消耗、自选模型传入请求体、thinking 关闭、闲聊省去强制工具轮、问号分句、原有异常/额度/历史/取消，以及受限改写成功和再次非法时只降级不循环。EditMode 作业 0d940ec425c2459abd094c86e8bb221f 为 36/36 Passed（Feedback/guard-tests.json）。

**未验证或已知限制**：生成文案仍可能因严格字段/内容守卫而降级；本地屏蔽会明确显示，不冒充实时 AI 回复。未重新做完整胜利局、发布构建或全波次采样。本阶段的零请求测试覆盖服务层，不等同于 S10/S11 全场景验收。

**超出范围未做**：不改合同胜利完成规则、不调整掉落、不新增人设美术、不新增供应商。
...
## S16 自由聊天与按需事实查询（2026-09-13，待人工验收）

**实施内容**：营地普通闲聊改为独立的自然对话协议，只要求模型返回 `text`，不再强制 objectiveEcho、verdict、合同事实块或工具调用；会话历史仍只在当前营地会话保留。合同、目标、背包、进度等问题继续走事实路由和本地权威链路。小芯增加独立的自然聊天离线兜底，避免网络失败时回落为合同报告。

**安全边界**：新增自由文案校验，仅拦截富文本、占位符、提示词/实现细节、越权修改存档或奖励、以及内容规范禁区；普通数字、专名、引号和“下次”等自然表达不再被旧的机制句校验误杀。
...
## S17 反馈修复：营地上下文连续性（2026-09-13）

**阶段**：S17 验收反馈修复，未开始 S18。

**核心链路**：玩家已看到的开场/回复 → 当前营地内存历史 → 后续请求及失败重写请求。

**目的**：修复开场、重抽和回退导致的聊天上下文断点。

**技术选择**：保留真实显示的 assistant 开场，内部开场指令使用 system 且不存入历史；成功、改写和回退统一记录已经显示的文本。历史保存客户端替换后的文本，不保留会随新合同错绑的引用索引。重抽成功只加入合同变更提示，不重建服务；对话处理中拒绝重抽避免并发。历史仍只在内存，退出营地后新服务不继承。

**改动文件**：`Scripts/Npc/NpcDialogueService.cs`（历史记录与注入、开场路由、重写上下文）；`Scripts/Quest/CampController.cs`（开场可见历史、重抽生命周期）；`Editor/LLM/NpcConversationMemoryAudit.cs` 及 meta（真实请求构造回归）；本记录及 `Docs/Evidence/NpcExperience/S17/camp-memory-feedback.md`。

**验证结果**：Unity 编译后 0 错误；53/53 EditMode Passed；专用服务审计七项通过；真实 DeepSeek 三次请求均 failure=null，最后回复找回“咕噜”并承接开场的垫子/温水。执行入口、job ID、完整模型样本见上述证据文档。

**未验证或已知限制**：实际营地点击重抽后的对话仍需用户复测；真实模型仅采样一组，不证明所有长对话都准确。此前 S17 报告“问候不写入会话历史”是实现错误：正确边界是不伪造玩家发言，但小芯实际说出的开场应保留。原有 53 项测试不能独立证明聊天连续性，本次增加了请求级审计。

**超出范围未做**：不实现 S18 话题种子，不增加跨局聊天/个人偏好存档；不改行动记忆、掉落或合同判定，不改用户场景/字体资产。


## S18 可延续的营地小话题（2026-09-13，待人工验收）

**核心链路**：`NpcTopicCatalog` 已批准种子 → `NpcTopicState` 会话状态 → 小芯当前营地请求上下文。

**目的**：让小芯围绕一个具体小事提出可接续的方向，玩家接受、拒绝、逗弄或换题会改变后续上下文；状态不跨局、不进入行动存档。

**技术选择**：新增六个带世界边界的本地种子，包含情境、核心小事、态度、回应方向、可展开细节和限制；服务按会话选一个种子，记录拒绝状态、已展开轮次和玩家最近输入。拒绝后只提示模型不要重新推送，合同重抽仍保留话题状态。

**改动文件**：`Scripts/Npc/NpcPersona.cs`（种子与状态）；`Scripts/Npc/NpcDialogueService.cs`（注入和拒绝路由）；`Editor/LLM/NpcConversationMemoryAudit.cs`（请求级回归）；`Docs/Evidence/NpcExperience/S18/topic-seed-audit.md`。

**验证结果**：Unity 刷新、编译成功，控制台 0 错误；EditMode 53/53 Passed；专用审计覆盖批准种子、拒绝分支、上下文连续性、重抽提示、回退和会话隔离。

**未验证或已知限制**：尚未进行三条真实 UI 分支的人工采样；本阶段没有新增三个可点击话题按钮，仍以自由输入为主。未实现好感、奖励、跨会话昵称或个人偏好。

**超出范围未做**：不做营地 3D、不写行动记录、不做 S19 历史检索、不修改合同判定和掉落。


## S19 有来源的按需回忆（2026-09-13，待人工验收）

**核心链路**：历史意图路由 → `SelectHistoricalContext` 本地投影 → 带 `recordId`/来源的只读历史上下文 → 小芯自然回答。

**目的**：只有玩家明确询问过去经历时才检索本地行动记录，避免每轮闲聊注入整批存档，并将历史与当前合同/背包分开。

**技术选择**：扩展 `DialogueRouter` 的历史词识别；服务保留结构化历史对象，按上一趟、物品相关问题投影一条快照和最多三条相关记录；历史提示明确没有记录的字段不得补写，普通任务概览不进入历史路径。

**改动文件**：`Scripts/Npc/Core/NpcCore.cs`、`Scripts/Npc/NpcDialogueService.cs`、`Docs/Evidence/NpcExperience/S19/history-query-audit.md`。

**验证结果**：Unity 编译成功、控制台 0 错误；EditMode 53/53 Passed，0 failed，0 skipped；S17 请求级上下文审计仍通过。

**未验证或已知限制**：尚未做真实营地历史问题的人工采样；“最近发生了什么”目前不触发历史检索，仍由小芯近况自然回答；历史筛选首版按关键词和字段存在性，未做语义向量检索。

**超出范围未做**：不保存聊天原文，不写入新的存档格式，不增加跨局个人偏好，不做 S20 三面联调。


## S20 波次、结算表达与体验验收准备（2026-09-13，待人工验收）

**核心链路**：`WaveDirector.OnWaveStageChanged` → `WavePulseService` → `RequestPulseReplyAsync`；`GameSession.OnRunEnded` → `ResultView` → `RequestDebriefAsync`；三面共享小芯默认人设与本地事实边界。

**改动内容**：将局内无线电脉冲名牌从“调度员”统一为“小芯”，保留脉冲短句、取消和阶段过期保护；结算入口继续订阅本地结算事件，并由服务依据冻结快照生成汇报。

**验证结果**：Unity 编译成功，控制台 0 错误；EditMode 53/53 Passed；链路和签名核对见 `Docs/Evidence/NpcExperience/S20/three-surfaces-audit.md`。

**未验证或已知限制**：尚未进行真实局内波次与胜利/死亡结算的人工三面采样；这一步需要用户运行一次完整或可触发的局内流程。自动测试不能证明语气、长度和死亡表达体验。

**超出范围未做**：不改波次压力、结算判定、掉落和存档；不做营地 3D；不增加跨面聊天历史。


## S19 反馈修复：历史同义问法路由（2026-09-13）

**问题**：玩家询问“之前的历史记录”被路由为普通闲聊，未注入历史来源，因此小芯回答不知道。

**修复**：历史意图新增“历史、之前、过去、曾经、哪一局、哪次”等中文同义词；这些表达与“上一趟/上次/经历”一致，进入按需历史事实路径，普通“最近发生了什么”仍保持近况闲聊。

**验证结果**：新增路由单测；EditMode 54/54 Passed，0 failed，0 skipped；Unity 控制台 0 错误；job `1288882597444225a1d15dd2e43b6cbe`。


## S19 反馈修复：最高带回价值口径（2026-09-13）

**核对结果**：工程已有 `SaveData.bestBackpackValue`，由胜利结算时以 `RunResult.BackpackValue` 更新；主菜单已有“最高背包价值”展示。此前将“最高记录”视为无统一指标的判断不准确。
...
## S11 反馈修复：结算评语被按钮遮挡（2026-09-14）

**阶段**：S11 · 结算任务区与汇报的布局修复。

**核心链路**：Result Builder → 生成主流程结算面板 → 自动换行/内容自适应 → 独立文本视口滚动。

**目的**：完整查看小芯评语及带出清单，避免内容被底部按钮遮挡。

**技术选择**：扩大对话框并重新排列统计区；评语优先显示，ScrollRect、RectMask2D、VerticalLayoutGroup 与 ContentSizeFitter 管理动态长文。新增只更新结算的 Builder 入口，避免重建暂停菜单。沿用 ResultView 的现有文本引用和异步赋值，不改变结算判定。

**改动文件**：`Editor/V04RunMenusArtBuilder.cs`（布局和单独生成入口）；`Scenes/Run/01-Run_ArtFull.unity`（实际 UI 与绑定）；`Docs/Evidence/ResultLayout/`（截图和验证读数）；本复盘记录。

**验证结果**：Unity 编译后控制台 0 error；长评语 6 行实际高度与首选高度一致；40 项清单可经 OnScroll 滚到底，底部误差 0；三种窗口尺寸的缩放边界核算无碰撞。已目视检查 Game View 截图。步骤、具体数值和首次检查不足见 `Docs/Evidence/ResultLayout/verification.md`。

**未验证或已知限制**：验证使用编辑态真实场景临时填充内容，未再打一局、未请求模型；三种尺寸为边界核算，不声称实际切换屏幕试玩。旧 01-Run 场景未重新生成。

**超出范围未做**：任务数值、存档、NPC 提示、波次、暂停菜单和既有按钮行为。本轮保留用户原有场景/配置/字体改动。

## 结算评语情绪化修复（2026-09-14）

**问题**：结算页评语原先强调“本地结算已完成、结果以本地判定为准”，并要求安静汇报，导致胜利和失败都像事实播报，缺少小芯的情绪与角色存在感。

**修复**：结算提示改为回到营地后的第一句话。胜利路径要求小芯表达开心、松一口气和照料主人的愿望；失败路径要求先担心、心疼和陪伴，不责备、不说教。事实只允许用一两个已确认结果作点缀，不列统计、不复读合同。ResultView 的网络等待占位语也改为带角色动作的短句，避免等待时显示系统术语。

**改动文件**：`Scripts/Npc/NpcPersona.cs`（SettlementTone）；`Scripts/Npc/NpcDialogueService.cs`（结算场景提示与成功/失败分流）；`Scripts/Presentation/Result/ResultView.cs`（成功/失败占位文案）；`Editor/V04RunMenusArtBuilder.cs`（默认占位文案）。

**验证结果**：Unity 编译完成，控制台 0 error；EditMode 58/58 Passed，0 failed，0 skipped，job `ecc10300d0cd450abae944b933cb3ad2`。

**未验证或已知限制**：本次尚未用真实 DeepSeek 分别跑胜利和失败结算对话；最终情绪强度、句长和事实引用比例仍需用户试玩反馈。

**超出范围未做**：未修改本地合同判定、统计字段、结算布局、营地记忆和波次脉冲。

## S21 数值与实施遗漏收尾（2026-09-16）

**阶段**：S21 · balance-2 配置修正。

**核心链路**：版本化资产/初始 Builder → 新合同冻结副本 → 本地条件判定。

**目的**：补齐 T1 精英 30 和 T5 300/220/180 三档，纠正 type=12 被误称开箱的问题，消除文档与资产矛盾。

**技术选择**：保留 T1 总击杀 900 并增加精英 30；T5 战斗型 300 + 价值 100,000，资源型 220 + 价值 100,000 + 精确 Epic 开箱 4，构筑型 180 + 价值 110,000 + Lv3 物品；三者都要求任务物品任选 1。definitionVersion=balance-2；不迁移已冻结 pendingQuest，不清档。新建默认配置与真实资产同步，Builder 已有数据库优先规则不变。

**改动文件**：15 个 `Data/Quest/prototype-*.asset`；`Editor/QuestCatalogBuilder.cs`；`Editor/LLM/QuestBalanceAudit.cs` 及 meta；两份技术/施工文档与本复盘；`Docs/Evidence/QuestBalance/balance-2-audit.txt`。

**验证结果**：MCP 调用 QuestBalanceAudit.Run：PASS，覆盖 15 个真实资产、8 份精英目标的差 1/恰好/死亡、T5 完整组合、品质精确匹配、任务池可获得性、旧版条件 JSON 保留与复制隔离。Unity 控制台 0 error；EditMode 58/58 Passed，job a691d86afec44a848beda2463c5ec1b4。通过本机已配置的 MCP HTTP 端点连接，无额外依赖。

**未验证或已知限制**：556 是单局精英击杀，不是多局生成均数；1,283 是不受阻连续时间理想期望。无生成统计不能推出真实生成均数；多局“偏难”及任务物品出现率尚待验收。概率池可获得不等于每局保证出现。

**超出范围未做**：存档自动迁移、波次生成、掉落权重、定向保底、更多全新合同类型、场景与字体；S16–S20 状态按既有证据修正，不一概标记完成。
