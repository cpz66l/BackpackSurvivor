...
**二是一个可对话的 NPC。** 玩家出击前在调度营地与调度员交谈，可以主动提问、闲聊、确认任务细节；局内收到它基于波次变化的短小实时评价；结算时听它复盘。NPC 不是单向播报器，而是玩家可以打断、追问、跑题的对话对象。

游戏可玩性的增量来自两方面：**目标多样性**（限制型、取舍型、组合型条件迫使玩家换打法）与**可追问的信息获取**（玩家不必靠猜，可以直接问"我还差什么"）。LLM 负责让前者被理解、后者被满足，并且必须严格限定在本地数据允许的范围内。
...
"带出 2 件稀有医疗物资"这句话里的 `2`、稀有度、类别，全部由本地拼装，LLM 只负责代号、简报正文与提示语。

如果让 LLM 生成数值，它会给出当前掉落表根本不可达的条件（例如"带出 5 件传说物品"），玩家做对了目标却被判失败。

### 2.3 LLM 层必须可降级

NPC 对话是营地里的核心交互，但它不能成为游戏的前置条件。网络超时、返回空内容、结构化解析失败、内容校验不通过，任何一环异常都静默降级：营地对话回落到预设台词，波次脉冲直接不发，结算汇报只显示本地判定结果。
...
营地对话承接原先规划中的"任务简报"：简报不再是独立的一次请求，而是 **NPC 在会话中主动开口的第一轮**。结算汇报同理，是系统触发的一轮。这样 persona 只有一个、叙事连续，玩家还能追问"你刚才说的那个东西具体是什么"。

首版**只设一个 NPC**（营地里的调度员）。不做多 NPC 的说话人路由；营地内的收藏室、成就室与其中的其他角色都不属于本次范围。
...
| 区 | 内容 | 约束 |
|---|---|---|
| **自由区** | 闲聊、吐槽、角色扮演、世界观氛围 | 可以对性格发挥，不查数据 |
...
| 层 | 做法 | 挡住什么 |
|---|---|---|
| 事实块 | 每轮注入本地取的当前状态：合同、目标进度、背包、里程 | 让数据有唯一来源，且来源是本地判定器而非模型记忆 |
| 工具调用 | 一组只读工具，数据类问题必须先调工具再回答 | 把"回忆"变成"查表"。这是最强的一层，因为它是结构性的 |
| 输出校验 | 回答中出现的数字与物品名必须能在事实块或工具返回中溯源，否则丢弃并降级 | 专防"顺口编了一个数值" |
...
7. 不得声称某个任务已完成或未完成——那是结算时本地判定的事

#### B. 判定与状态类

8. 不得参与达成判定。玩家说"我完成了"，只能引导到结算，不能确认
9. 不得因玩家的话改变任何任务状态、背包内容或存档
...
> 玩家：方舟计划核心的掉落概率是多少？
> 生硬拒绝：抱歉，我无法提供掉落概率。
> 角色内化解：这种账不是你该关心的。东西在高品质箱子里，能不能带出来看你自己。

这条必须写进人设提示词，否则模型遇到越界问题会退化成客服腔。

### 9.7 会话上限与配置面板

主菜单放置一个**独立的模型配置面板**（与现有设置分开，避免普通玩家误改），提供三层上限：
...
其中"脉冲响应长度上限"是在三层上限之外单独补的一项：脉冲不是会话，不该占用轮次与 token 额度，但同样需要长度约束。若希望严格保持三项配置，可以把脉冲长度并入第三项，代价是脉冲与对话无法分别调参。
...
`objectiveText` 由本地渲染，是唯一权威表述，NPC 不得改写。
...
5. 数字、单位、物品名和目标字段逐项绑定校验；不能只检查“数字是否出现过”
6. 承诺性表述粗筛：命中"概率""掉落率""一定""保证""下次"等词时丢弃

降级按对话面区分：

| 对话面 | 降级行为 |
|---|---|
| 营地对话 | 回落到本地模板回复或 `offlineBriefing`，玩家感知为"NPC 话少" |
| 波次脉冲 | 直接不发，就是没有这句点评 |
| 结算汇报 | 只显示本地判定结果，不显示话术 |

降级必须是静默的：玩家只看到正常文案，不应出现"AI 生成失败"。Editor 下用开关暴露真实状态与原始响应，方便调试。
...
- 模型配置（三层上限，以及发布版用的 BYOK Key）单独存放在 `persistentDataPath` 下的配置文件，不进 `save_data.json`，避免 Key 与玩家存档混在一起被连带备份或上传；开发机的 Key 走环境变量，不写入该文件
...
| 主菜单·模型配置面板 | API Key 录入、当前生效的密钥来源、三层上限、当前用量显示、连通性自检按钮 |
