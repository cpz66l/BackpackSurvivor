...
### BUG-000B · Die() 里找不到 lootManager（CS0120）
...
## BUG-009 · 拖拽丢弃激活武器后，新顶上的武器 UI 标记不刷新
...
## BUG-011 · HUD 血条 Slider 被 A/D 移动键控制
...
### BUG-014 · 字段改名导致 GoldOrb 池引用丢失，敌人死亡掉落时报 NRE
...
- **修复**：将伤害数字显示从 `Mathf.CeilToInt(damage)` 改为不向上夸大显示；后续统一评估“伤害源头整数化”或“UI 显示一位小数”的最终方案。
- **沉淀规则（一句话）**：战斗反馈不能 overpromise；显示值和真实规则必须语义一致，否则玩家会觉得系统不讲理。

---

### BUG-016 · Player 血量序列化为 0 导致敌人生成后不追击
...
### BUG-017 · 旋转后接口 UI 正确但 ScanAdjacency 不跟随触发
...
### BUG-020 · Build Settings 残留旧 Run1 场景路径
...
### BUG-021 · 主菜单 UI 分辨率变化后偏移，Scrollbar 死活不好用
...
### BUG-026 · 主菜单开始游戏偶发无反应

- **日期**：2026-08-18 · **所属系统**：主菜单 / UI 音效 / 场景流（MainMenuController）
- **现象**：主菜单点击“开始游戏”后，偶尔没有进入 `01-Run`，表现为按钮像是点了但主流程没有继续。
- **复现步骤**：① 打开 MainMenu 场景 ② 点击开始游戏 ③ 偶发留在主菜单，或需要再次点击才进入 Run 场景。
- **排查过程**：先检查 Build Settings，确认 `MainMenu` 和 `01-Run` 都已加入构建且场景名正确；再检查 StartButton 的点击链路，发现为了保留 UI 点击音，开始游戏从立即 `LoadScene` 改成了协程延迟 0.08 秒后加载，并用 `isLeavingScene` 阻止重复点击。该方案把核心入口可靠性绑定到了协程和当前 UI 对象生命周期上。
- **根因**：开始游戏按钮作为主流程入口，依赖“先播点击音、等待协程、再切场景”的异步链路；一旦边缘情况下协程未按预期推进或状态锁提前拦截后续点击，就会表现为偶发无反应。
- **修复**：开始游戏改回点击后立即 `SceneManager.LoadScene("01-Run")`；点击音改为创建一次性 `OneShotUIButtonAudio`，使用 `DontDestroyOnLoad` 跨场景播放，播完自动销毁。这样音效不再阻塞主流程入口。
- **沉淀规则（一句话）**：核心场景流不能为了等待 UI 音效牺牲可靠性；需要跨场景保留声音时，用一次性音源兜底，而不是延迟主流程。
