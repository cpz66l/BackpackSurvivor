...
{
    [InitializeOnLoad]
    public static class QuestTrackerFeedbackAudit
    {
        const string Active = "BS.Quest.TrackerFeedbackAudit";
        static int step;
        static double next, deadline;
        static string Dir => Path.GetFullPath(Path.Combine(Application.dataPath, "../../Docs/Evidence/Feedback"));
        static QuestTrackerFeedbackAudit()
        {
        ...
        [MenuItem("Tools/Backpack Survivor/Quest/S9 Verify Tracker Feedback")]
        public static void Run()
        {
            ...
            File.WriteAllText(Path.Combine(Dir,"tracker.txt"), DateTime.UtcNow.ToString("O")+"\n");
            ...
            try
            {
                ...
                if (step == 1)
                {
                    ...
                    var q = new QuestInstance { eventId="tracker-feedback", tier=2, briefingTitle="追踪验证",
                        objectives=new System.Collections.Generic.List<ObjectiveClause> { new ObjectiveClause { type=ObjectiveType.CarryItem, itemId="测试物品", count=1 } } };
                    string error;
                    ...
                    var item = new Item("测试物品", Rarity.Common, 1, 1, ItemTag.Medical, ConnectableSides.None, 10, 0, 1);
                    session.PauseRun();
                    Check(grid.Place(0,0,item) && tracker.DisplayText.Contains("✓"), "paused inventory add checks target immediately");
                    grid.Remove(item);
                    Check(!tracker.DisplayText.Contains("✓"), "paused inventory remove clears target immediately");
                    grid.Place(0,0,item); session.ResumeRun(); Time.timeScale = 0;
                    Note("HUD="+tracker.DisplayText.Replace("\n"," | "));
                    ScreenCapture.CaptureScreenshot(Path.Combine(Dir,"tracker-target.png"));
                ...
                else if (step == 2)
                {
                    ...
                    ScreenCapture.CaptureScreenshot(Path.Combine(Dir,"tracker-death.png"));
                    step=3; next=EditorApplication.timeSinceStartup+1;
                }
                else Finish("PASS tracker lifecycle; injected order/item/outcome; isolated temporary save");
            }
            catch (Exception e) { Finish("FAIL "+e); }
        }
        static void Check(bool pass,string why) { if(!pass)throw new InvalidOperationException(why);Note("PASS "+why); }
        static void Note(string s) { File.AppendAllText(Path.Combine(Dir,"tracker.txt"),s+"\n"); }
