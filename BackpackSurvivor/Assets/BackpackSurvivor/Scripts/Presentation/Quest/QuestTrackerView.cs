...
namespace BS.Presentation
{
    public sealed class QuestTrackerView : MonoBehaviour
    {
        ...
        void OnEnable(){if(session==null) session=FindAnyObjectByType<BS.GamePlay.Run.GameSession>(); if(session!=null){session.OnStateChanged+=OnState;session.OnTimeChanged+=OnTime;session.OnXpChanged+=OnXp;} BS.GamePlay.Loot.LootChest.OnOpened+=OnChest; BS.GamePlay.Loot.DropItem.OnCollected+=OnItem;}
        void OnDisable(){if(session!=null){session.OnStateChanged-=OnState;session.OnTimeChanged-=OnTime;session.OnXpChanged-=OnXp;} BS.GamePlay.Loot.LootChest.OnOpened-=OnChest; BS.GamePlay.Loot.DropItem.OnCollected-=OnItem;}
        ...
        void OnTime(float a,float b){RefreshSnapshot();}
